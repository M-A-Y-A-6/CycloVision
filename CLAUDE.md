...
Front-end-only demo prototype for a YouTube pitch video for **Smart India Hackathon 2026** (Team Formula 1, Problem Statement **SIH26070**: an AI/ML system for identification, classification and prediction of tropical cyclone patterns using multi-source satellite data). All data and AI outputs are synthetic and pre-written (seeded, so identical every run) — there is no backend.
...
**Dashboard** (`/`): logo, tagline, the disclaimer, 4 historical-case cards (name, year, thumbnail, basin, IMD category), then `[ Simulate ]`, disabled until picked. Picking a card commits nothing until Simulate is clicked (`setCase`, `start()`, navigate to `/analysis`).
...
`src/data/cases.ts` exports `CASES`: 4 `HistoricalCase`s, real storm names/basins (a historical fact) with every other number/image/output synthetic — **Remal** (2024, Bay of Bengal, Moderate risk), **Michaung** (2023, Bay of Bengal, Low), **Biparjoy** (2023, Arabian Sea, High), **Tauktae** (2021, Arabian Sea, High, `DEFAULT_CASE_ID`). A self-check throws on import if any case's numbers stop adding up. Any screen naming a case must show, once, quietly: **"Illustrative simulation for demo purposes — not the verified historical record."** (`DISCLAIMER`; `StageLayout` shows it automatically for every story stage).

Each case has `seed`, `center`, `movement`, `currentIntensity`, `geometry` (eyewall ring + hotspots — the same array feeds both the rendered satellite image and `explainability.gradCamHotspots`), `ensemble` (30-member wind-speed paths), and a `precomputed` object:
...
- Show the **"Synthetic demo data"** chip in the top bar at all times; one plain-language sentence per stage.
...
- Story stages and Dashboard need **no page scrollbar** at 1920x1080 down to 1280x720 (use `short:`, active ≤820px height). `/results`, `/dataset` and `/about` are document-style pages, read after or outside the animated story, and may scroll.
...
src/
  ...
  stages/      Detect, Identify, Classify, Predict, Result — one per story stage; read the case via lib/useCase.ts
  pages/       Dashboard (/), Analysis (/analysis, redirects to / if no case set), ResultsPanel (/results, guarded by resultsPanelReached; tabs in pages/results/: ResultsTab, ExplainTab, TrackTab, ConsensusTab, AlertTab), DatasetPage (/dataset, the fictional "VarunSet" dataset), AboutPage (/about, a Model Card), sharing DocumentHeader
  lib/         cn, spiral, random (mulberry32), noise, satellite (procedural generator, per-case geometry), satelliteWorker + satelliteBitmap (off-thread imagery, cached by case), swirlSize, useBeats, useElementSize, useCase
  data/        cases.ts — 4 HistoricalCases + self-check (see Data)
  store/       story.ts — zustand: stage, currentCaseId/setCase, doneStages/markDone/view (tracker), resultsPanelReached/reachResultsPanel, start/advance/reset
  config/      timings.ts, stages.ts (titles/sentences/tracker labels), map.ts, imagery.ts (prewarm list), risk.ts / basin.ts (badge colours), app.ts
