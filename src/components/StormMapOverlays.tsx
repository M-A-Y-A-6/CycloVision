...
/**
 * A soft swirl of the storm's own cloud (the water-vapour image) sitting on the map at the case's position.
 * progress 0 = invisible, 1 = fully built. Must be used inside <MonitoringMap>.
 */
export function StormSwirl({ caseData, progress }: { caseData: StormCase; progress: number }) {
  const fade = 'radial-gradient(circle closest-side, #000 35%, transparent 100%)'
  return (
    <MapAnchor lat={caseData.center.lat} lon={caseData.center.lon} className="mix-blend-screen" zIndex={455}>
      <motion.div
        initial={false}
        animate={{
      ...
      >
        <SatelliteCanvas channel="wv" size={SWIRL_IMAGE_PX} caseData={caseData} className="size-full" />
      </motion.div>
    </MapAnchor>
  )
}
...
export function DetectionMarker({ caseData, show }: { caseData: StormCase; show: boolean }) {
  return (
    <MapAnchor lat={caseData.center.lat} lon={caseData.center.lon} zIndex={470}>
      <motion.div
        initial={false}
